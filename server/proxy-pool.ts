...
// ═══════════════════════════════════════════════
//  THAI PROXY POOL — Dedicated Thai IP Proxies
//  For geo-cloaking detection (sites that redirect
//  only Thai IPs to gambling/slot sites)
// ═══════════════════════════════════════════════

const RAW_THAI_PROXY_LIST = `
154.91.201.64:44001:ujk6929555692eb1:QFaweUht0yMm92zj8t
62.112.141.247:44001:leX68a3123f423e2:Oz1QnyM0a9vKufm7wD
62.112.140.202:44001:fQk693944a3949e9:m08iLaGXYWoMSqnH1N
62.112.140.52:44001:rNP69227b09ca64b:S3FIHAYVBsqyvdaWRZ
...
/**
 * Thai Proxy Pool — Separate pool for Thai residential IPs
 * Used specifically for geo-cloaking detection where sites
 * redirect only Thai IPs to gambling/slot content.
 */
class ThaiProxyPool {
  private proxies: ProxyEntry[] = [];
  private roundRobinIndex = 0;
  ...
  private initialize(): void {
    ...
    const rawList = process.env.THAI_RESIDENTIAL_PROXIES || RAW_THAI_PROXY_LIST;
    const lines = rawList.split("\n").map(l => l.trim()).filter(l => l && !l.startsWith("#"));
    ...
    this.proxies = lines.map((line, idx) => {
      ...
      return {
        id: 10000 + idx, // offset to avoid collision with main pool
        ip,
        port,
        username,
        password,
    ...
    this.initialized = true;
    console.log(`[ThaiProxyPool] Initialized with ${this.proxies.length} Thai residential proxies`);
  }

  /**
   * Get a random healthy Thai proxy
   */
  getProxy(strategy: RotationStrategy = "random"): ProxyEntry | null {
    const healthy = this.proxies.filter(p => p.healthy);
    if (healthy.length === 0) {
      // Reset all to healthy and try again
      this.proxies.forEach(p => p.healthy = true);
      const all = this.proxies;
      if (all.length === 0) return null;
      return all[Math.floor(Math.random() * all.length)];
    ...
    switch (strategy) {
      ...
      case "least-used": {
        const selected = healthy.reduce((a, b) =>
          (a.successCount + a.failCount) <= (b.successCount + b.failCount) ? a : b
        );
        selected.lastUsed = Date.now();
        return selected;
  ...
  /**
   * Get N unique Thai proxies
   */
  getMultipleProxies(count: number): ProxyEntry[] {
    const healthy = this.proxies.filter(p => p.healthy);
    if (healthy.length === 0) return [];
    const n = Math.min(count, healthy.length);
    const shuffled = [...healthy].sort(() => Math.random() - 0.5);
  ...
  reportResult(proxyId: number, success: boolean, latencyMs?: number): void {
    ...
    if (total >= 5 && proxy.failCount / total > 0.8) {
      proxy.healthy = false;
      console.log(`[ThaiProxyPool] Proxy ${proxy.label} marked unhealthy (${proxy.failCount}/${total} failures)`);
    }
  }
  ...
  getStats(): ProxyStats {
    ...
    return {
      ...
      healthy: healthy.length,
      unhealthy: this.proxies.length - healthy.length,
      avgLatencyMs: healthy.length > 0 ? Math.round(healthy.reduce((s, p) => s + (p.avgLatencyMs || 0), 0) / healthy.length) : 0,
      totalRequests,
      successRate: totalRequests > 0 ? Math.round((totalSuccess / totalRequests) * 100) : 100,
    };
  }
  ...
  get healthyCount(): number { return this.proxies.filter(p => p.healthy).length; }
}

export const thaiProxyPool = new ThaiProxyPool();

/**
 * Fetch using a Thai residential proxy.
 * Used for geo-cloaking detection — simulates a Thai user accessing the site.
 * Adds Thai Accept-Language header automatically.
 * 
 * Returns null if all Thai proxies fail (does NOT fallback to direct/foreign proxy).
 */
export async function fetchWithThaiProxy(
  url: string,
  init: RequestInit = {},
  options: {
    timeout?: number;
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    const proxy = thaiProxyPool.getProxy("round-robin");
    if (!proxy || triedIds.has(proxy.id)) {
      // Try to find an untried proxy
      const untried = thaiProxyPool.getMultipleProxies(50).filter(p => !triedIds.has(p.id));
      if (untried.length === 0) break;
      const altProxy = untried[0];
      triedIds.add(altProxy.id);

      const result = await _tryThaiProxyFetch(url, init, altProxy, timeout);
      if (result.success && result.response) {
        return { response: result.response, proxyUsed: altProxy };
      }
      continue;
    }

    triedIds.add(proxy.id);
    const result = await _tryThaiProxyFetch(url, init, proxy, timeout);
    if (result.success && result.response) {
      return { response: result.response, proxyUsed: proxy };
    }
  }

  console.log(`[ThaiProxy] All ${triedIds.size} Thai proxies failed for ${url.substring(0, 80)}`);
  return null;
}

/**
 * Internal: Try fetch with a single Thai proxy
 */
async function _tryThaiProxyFetch(
  url: string,
  init: RequestInit,
  proxy: ProxyEntry,
  timeout: number,
  ...
  try {
    ...
    const latencyMs = Date.now() - start;
    thaiProxyPool.reportResult(proxy.id, true, latencyMs);
    ...
    const latencyMs = Date.now() - start;
    thaiProxyPool.reportResult(proxy.id, false, latencyMs);
    return {
      success: false,
      error: err instanceof Error ? err.message : String(err),
      latencyMs,
...
/**
 * Get Thai proxy pool stats
 */
export function getThaiPoolStats(): { total: number; healthy: number; dead: number; avgSuccessRate: number } {
  const stats = thaiProxyPool.getStats();
  return {
    total: stats.total,
    healthy: stats.healthy,
    dead: stats.unhealthy,
